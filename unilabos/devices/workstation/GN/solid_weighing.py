...
class SolidWeighingDevice(OpcUaClientWithSubscription):
    ...
    @not_action
    def _wait_idle_state(self, timeout: float = _IDLE_GATE_TIMEOUT) -> None:
        """下发前确认 PLC 空闲：CompleteFB=1 且触发/类型已清零。"""
        logger.info(
            f"等待设备空闲（{self.COMPLETE_NODE}=1，"
            f"{self.CMD_TRIG_NODE}=0，{self.CMD_TYPE_NODE}=0）..."
        )
        start = time.monotonic()
        while time.monotonic() - start < timeout:
            complete = self._opc_read(self.COMPLETE_NODE, force_read=True)
            trig = self._opc_read(self.CMD_TRIG_NODE, force_read=True)
            cmd = self._opc_read(self.CMD_TYPE_NODE, force_read=True)
            if complete == 1 and trig == 0 and cmd == 0:
                logger.info("✓ 设备已空闲，可以下发命令")
                return
            time.sleep(0.1)
        complete = self._opc_read(self.COMPLETE_NODE, force_read=True)
        trig = self._opc_read(self.CMD_TRIG_NODE, force_read=True)
        ...
        raise ValueError(
            ...
            f"{self.CMD_TRIG_NODE}={trig!r}, "
            f"{self.CMD_TYPE_NODE}={cmd!r}"
        )
    ...
    def _trigger_and_wait(
        ...
        initial_complete = self._opc_read(self.COMPLETE_NODE, force_read=True)
        if initial_complete != 1:
            raise ValueError(
                f"{description}下发前 {self.COMPLETE_NODE}={initial_complete!r}，"
                "期望空闲状态 1"
            )
        ...
        try:
            ...
            if completed:
                ...
                logger.warning(f"{description}未完成，跳过 CmdTrig/CmdType 清零")
