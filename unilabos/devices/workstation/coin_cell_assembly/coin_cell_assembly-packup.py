...
#
    ...
    def func_pack_device_start(self) -> None:
        #切换手动模式
    ...
    def func_pack_device_auto(self) -> Dict[str, Any]:
        #切换自动
  ...
  # ====================== 命令类指令（REG_x_） ======================
    ...
    def func_pack_read_data_cmd(self, file_path: str):
        if self.sys_auto_status and self.sys_start_status:
            self.csv_export_thread = threading.Thread(target=self.read_data_and_output(), args=(file_path,))
...
if __name__ == "__main__":
    ...
    CoinCellAssemblyWorkstation(Resource("1", 1, 1, 1))
    print("success")